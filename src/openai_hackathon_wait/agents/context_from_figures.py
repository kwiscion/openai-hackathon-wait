...
class FigureExtractorContext:
    ...
    figures_content: Optional[Dict[str, bytes]] = None  # Figure filename -> binary content
        ...
        if self.figures_content is None and self.figure_paths:
            ...
            for fig_path in self.figure_paths:
                try:
                    ...
                    print(f"Warning: Failed to read figure from path {fig_path}: {str(e)}")

def dynamic_instructions(wrapper: RunContextWrapper[FigureExtractorContext], agent: Agent[FigureExtractorContext]) -> str:
    """
    ...
    data_points_instruction = "Also extract specific data points from charts and graphs." if ctx.extract_data_points else ""
...
@function_tool
def analyze_figure(wrapper: RunContextWrapper[FigureExtractorContext], figure_path: str) -> str:
    """
    ...
    detail_instruction = "Provide a detailed analysis" if detailed else "Provide a concise overview"
    data_point_instruction = "Extract and list specific data points visible in charts/graphs." if extract_data else ""
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            },
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": f"Analyze this figure from an academic paper.\n\nContext: {context_text}\n\nPlease structure your response with the following sections:\n1. Figure Type\n2. Main Message\n3. Key Elements\n4. Significance\n5. Limitations/Alternative Interpretations"},
...
@function_tool
def extract_data_from_chart(wrapper: RunContextWrapper[FigureExtractorContext], figure_path: str) -> str:
    """
    ...
    if figure_path in figure_results:
        ...
        if not any(keyword in figure_type for keyword in ["graph", "chart", "plot", "histogram"]):
...
@function_tool
def detect_figure_relationships(wrapper: RunContextWrapper[FigureExtractorContext]) -> str:
    """
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                },
                {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": "Create an accessibility text description for this scientific figure. The description should enable a vision-impaired reader to understand what the figure shows, including key data, trends, and visual elements. Make your description thorough but concise."},
                        {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{base64_image}"}}
                    ]
                }
            ],
...
async def run_figure_extractor(figure_paths=None, figures_content=None, paper_path=None, paper_content=None, detailed_analysis=True, extract_data_points=False):
