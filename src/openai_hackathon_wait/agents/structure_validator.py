...
class PaperValidatorContext:
    ...
    paper_type: str = (
        "research"  # Options: "research", "review", "case_study", "thesis"
    )
    focus_areas: Optional[list[str]] = (
        None  # e.g. ["methodology", "results", "discussion"]
    )
    auto_detect_paper_properties: bool = (
        True  # Whether to auto-detect paper type and focus areas
    )
...
def dynamic_instructions(
    wrapper: RunContextWrapper[PaperValidatorContext],
    agent: Agent[PaperValidatorContext],
) -> str:
    """
    ...
    if ctx.auto_detect_paper_properties:
        auto_detect_instruction = (
            " First analyze the paper properties to determine its type and focus areas."
        )
...
async def run_validator(
    ...
    context = PaperValidatorContext(
        ...
        if paper_type
        else "research",  # Will be overridden if auto_detect is True
...
async def main():
    ...
    output1 = await run_validator(
        paper_path=markdown_path, auto_detect=True, min_quality_level="Average"
    )
